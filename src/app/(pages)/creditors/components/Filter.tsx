...
import { z } from 'zod';
import { getAllCreditors, getAllCreditorsFiltered } from '../actions';
import { CreditorsState } from './Table';
...
interface PropsType {
  ...
  setCreditors: React.Dispatch<React.SetStateAction<CreditorsState>>;
}
  ...
  const formSubmit = async (): Promise<void> => {
    try {
      ...
      const response = searchText
        ? await getAllCreditorsFiltered({
            page: 1,
            itemsPerPage,
            filters: { searchText },
          })
        : await getAllCreditors({ page, itemsPerPage });

      if (response?.error) {
        toast.error(response?.message || 'Error fetching creditors');
      } else {
